...
// 茶养首页
// ═══════════════════════════════════════════════════
async function renderShop(){updateThemeIcon();try{var a=await api('/shop/products');var h='<div class="header"><h1><i class="fa-solid fa-store"></i> 养生商城</h1><p>精选养生好物</p></div><div class="page">';for(var i=0;i<a.length;i++){var x=a[i];h+='<div class="recipe-card" onclick="openPrd('+x.id+')"><div class="recipe-name">¥'+x.price+' '+x.name+'</div><div style="font-size:12px;color:var(--text2)">'+(x.description||'').substring(0,50)+'...</div></div>';}h+='</div>';document.getElementById('app').innerHTML=h+nav('shop');}catch(e){document.getElementById('app').innerHTML='<div class="page"><div style="padding:40px">加载失败</div></div>'+nav('shop');}}
...
function renderShopProduct(){var p=state.pageParams.product;var h=hd(p.name);var b='<div class="page"><div class="card"><div style="font-size:28px;font-weight:700;color:var(--green)">¥'+p.price+'</div>'+(p.original_price&&p.original_price>p.price?'<span style="font-size:13px;color:#999;text-decoration:line-through">原价¥'+p.original_price+'</span>':'')+'<div style="font-size:16px;font-weight:600;margin-top:8px">'+esc(p.name)+'</div><div style="font-size:13px;color:var(--text2);line-height:1.6;margin-top:8px">'+(p.description||'')+'</div><button class="btn btn-primary btn-block btn-lg mt-4" onclick="addToCart('+p.id+')">加入购物车</button></div></div>';document.getElementById('app').innerHTML=h+b;}
async function renderShopCart(){updateThemeIcon();try{var items=await api('/shop/cart');var h=hd('购物车');if(!items||!items.length){document.getElementById('app').innerHTML=h+'<div class="page"><div style="padding:60px;text-align:center;color:var(--text2)">购物车是空的</div></div>';return;}var html='<div class="page">';var total=0;for(var i=0;i<items.length;i++){var it=items[i];total+=it.price*it.quantity;html+='<div class="card"><div class="flex-between"><div><div style="font-weight:600">'+it.name+'</div><div style="font-size:12px;color:var(--text2)">¥'+it.price+'×'+it.quantity+'</div></div><button class="btn btn-sm btn-outline" onclick="delFromCart('+it.id+')">删除</button></div></div>';}html+='<div class="card"><div style="font-size:20px;font-weight:700;color:var(--green);text-align:right">合计: ¥'+total+'</div><button class="btn btn-primary btn-block btn-lg mt-2" onclick="goCheckout()">立即结算</button></div></div>';document.getElementById('app').innerHTML=h+html;}catch(e){document.getElementById('app').innerHTML=hd('购物车')+'<div class="page"><div style="padding:40px">加载失败</div></div>';}}
...
function goCheckout(){var h=hd('结算');var b='<div class="page"><div class="card"><div class="form-group"><label>收货人</label><input id="ch-name" class="form-input" placeholder="请输入姓名"></div><div class="form-group"><label>手机号</label><input id="ch-phone" class="form-input" type="tel" placeholder="请输入手机号"></div><div class="form-group"><label>收货地址</label><textarea id="ch-addr" class="form-textarea" placeholder="请输入详细地址"></textarea></div><div class="form-group"><label>备注</label><input id="ch-remark" class="form-input" placeholder="选填"></div><button class="btn btn-primary btn-block btn-lg" onclick="submitOrder()">确认下单</button></div></div>';document.getElementById('app').innerHTML=h+b;}
