...
const SECRET = process.env.WELLNESS_SECRET || (() => {
  ...
  log.error('WELLNESS_SECRET 环境变量未设置！');
  process.exit(1);
})();
const PORT = 8000;
let db;
...
// Health
app.get('/api/health', function(req, res) { res.json({ status: 'ok', version: '0.1.0' }); });

// ── 微信支付 ──

// 获取支付参数（创建订单）
app.post('/api/pay/create', auth, async function(req, res) {
  try {
...
// 模拟支付完成（无商户号时使用）
app.post('/api/pay/mock-complete', auth, function(req, res) {
...
app.post('/api/auth/register', function(req, res) {
  ...
  queryRun('INSERT INTO users (username, hashed_password, nickname) VALUES (?, ?, ?)', [b.username, hashPw(b.password), b.nickname || b.username]);
  saveDb();
...
// 购买方案
app.post('/api/constitution/plans/:type/purchase', auth, function(req, res) {
  var plan = CONSTITUTION_PLANS[req.params.type];
  if (!plan) return res.status(404).json({ detail: 'Not found' });
...
// 用积分兑换
app.post('/api/points/redeem', auth, function(req, res) {
...
app.post('/api/vip/purchase', auth, function(req, res) {
...
// Orders
app.post('/api/shop/orders/create', auth, function(req, res) {
  var data = req.body || {};
...
// Book a chef
app.post('/api/chefs/book', auth, function(req, res) {
  var data = req.body || {};
...
async function start() {
  try {
    ...
    app.listen(PORT, '0.0.0.0', function() {
      log.info('服务已启动', { port: PORT, url: 'http://0.0.0.0:' + PORT });
    });
  } catch (e) {
    log.error('启动失败', e);
    process.exit(1);
